...
def save_trace_to_json(profiler):
    rank = torch.distributed.get_rank()
    json_trace_path = f"./trace_{rank}.json"
    profiler.export_chrome_trace(json_trace_path)
    print(f"Profiler trace saved to {json_trace_path}")
...
def pretrain(train_valid_test_dataset_provider,
    ...
    #     if torch.distributed.get_rank() == 0:
    #         os.environ['ROCBLAS_LAYER'] ='4'
    #         print_rank_0('*************** ROCBLAS_LAYER: {} ***************'.format(os.environ.get('ROCBLAS_LAYER')))
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    on_trace_ready=save_trace_to_json,  # 将 profiler 的 trace 保存为 JSON 格式
    record_shapes=True,
    with_stack=False,
